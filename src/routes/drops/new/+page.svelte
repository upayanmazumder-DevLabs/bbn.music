...
onMount(async () => {
	...
	try {
		...
		if (response.data) {
			...
			formState.compositionCopyright =
				drop.compositionCopyright || 'bbn.music';
			formState.soundRecordingCopyright =
				drop.soundRecordingCopyright || 'bbn.music';
			...
			formState.songs = drop.songs?.length
				? drop.songs.map((s: ApiSong) => {
						...
						if ('language' in s)
							songData.language = (s as any).language;
			...
			if (drop.artwork) {
				try {
					...
					if (artworkResponse.data) {
						formState.artworkPreview = URL.createObjectURL(
							artworkResponse.data as Blob,
						);
					}
				} catch {
...
// Derived values
const secondaryGenreOptions = $derived(
	getSecondaryGenres(formState.primaryGenre),
);
...
$effect(() => {
	if (
		formState.primaryGenre &&
		!secondaryGenreOptions.includes(formState.secondaryGenre)
	) {
...
async function saveDrop() {
	...
	try {
		...
		body.songs = formState.songs.map((song) => {
			...
			if (song.secondaryGenre)
				songData.secondaryGenre = song.secondaryGenre;
...
function saveArtist() {
	...
	const isDuplicate = formState.artists.some((artist, index) => {
		...
		if (editingArtistIndex !== null && index === editingArtistIndex)
			return false;
		...
		if (artist.type === tempArtist.type) {
			...
			if (
				tempArtist.name &&
				artist.name?.toLowerCase() === tempArtist.name.toLowerCase()
			)
				return true;
		}
		return false;
...
async function processSongFile(file: File) {
	...
	try {
		...
		const errorMsg =
			e instanceof Error ? e.message : 'Failed to upload song';
	...
	return new Promise((resolve, reject) => {
		...
		ws.onmessage = async ({ data }) => {
			if (data.startsWith('failed')) {
				...
				ws.send(
					`file ${JSON.stringify({ filename: file.name, type: file.type })}`,
				);
			} else if (data === 'next') {
...
async function uploadArtwork(file: File) {
	...
	try {
		...
		const errorMsg =
			e instanceof Error ? e.message : 'Failed to upload artwork';
		toast.show(errorMsg, 'error');
...
async function submitDrop() {
	...
	try {
		...
		const errorMsg =
			e instanceof Error ? e.message : 'Failed to submit drop';
...
<div class="min-h-screen max-w-4xl mx-auto px-4 py-8">
	...
	<div class="mb-10">
		<div class="flex items-center justify-between relative max-w-2xl mx-auto">
			...
			{#each steps as step}
				...
				<button
					type="button"
					class="relative z-10 flex flex-col items-center group transition-all duration-200 {isClickable ? 'cursor-pointer' : 'cursor-default'}"
				...
				>
					...
					>
						...
						{:else}
							<span class="text-sm font-semibold {isActive ? 'text-white' : 'text-gray-500'}">{step.id}</span>
						{/if}
					</div>

					<!-- Label -->
					<span class="mt-2 text-xs font-medium {isActive || isCompleted ? 'text-white' : 'text-gray-500'} transition-colors">
						{step.label}
					</span>
				</button>
			{/each}
	...
	<Card variant="glass" padding="lg">
			...
			<div class="space-y-8">
				...
				<div class="space-y-4">
					<div class="flex items-center justify-between">
						<div>
							...
							<p class="text-sm text-gray-400">Add primary artists, features, songwriters & producers</p>
						</div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
						{#each formState.artists as artist, index}
							<div class="group flex items-center gap-4 p-4 bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl border border-gray-700/50 hover:border-orange-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/10">
								<div class="w-12 h-12 rounded-xl bg-gradient-to-br from-{getArtistTypeColor(artist.type)}-500 to-{getArtistTypeColor(artist.type)}-600 flex items-center justify-center shadow-lg flex-shrink-0">
									<UserSolid class="w-6 h-6 text-white" />
								</div>
								<div class="flex-1 min-w-0">
									<p class="text-white font-semibold truncate">{artist.name || 'Unnamed Artist'}</p>
									<p class="text-sm text-gray-400 font-medium">{getArtistTypeLabel(artist.type)}</p>
								</div>
								<div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
									...
									>
										<TrashBinOutline class="w-4 h-4 text-gray-400 hover:text-red-400 transition-colors" />
									</button>
								</div>
							</div>
						{/each}
						...
						{#if formState.artists.length === 0}
							<div class="col-span-full text-center py-12 border-2 border-dashed border-gray-700 rounded-2xl">
				...
				<div class="space-y-4">
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<Select bind:value={formState.primaryGenre} label="Primary Genre" required error={formState.errors['primaryGenre']}>
						...
						<Select bind:value={formState.secondaryGenre} label="Sub-genre" required disabled={!formState.primaryGenre} error={formState.errors['secondaryGenre']}>
						...
						<Select bind:value={formState.language} label="Language" required error={formState.errors['language']}>
				...
				<div class="border-t border-gray-700 pt-6">
					...
					>
						...
						<ChevronDownOutline class="w-5 h-5 text-gray-400 transition-transform {showAdvancedSettings ? 'rotate-180' : ''}" />
					</button>
			...
			<div class="space-y-8">
				...
				<div class="flex justify-center">
					<div
						role="button"
						tabindex="0"
						class="relative w-96 h-96 rounded-3xl overflow-hidden {uploadingArtwork ? 'cursor-wait' : 'cursor-pointer'} group transition-all duration-500 {isDraggingArtwork
						...
						onkeydown={(e) => !uploadingArtwork && e.key === 'Enter' && document.getElementById('artwork')?.click()}
					>
						{#if uploadingArtwork}
							<!-- Loading overlay -->
							<div class="w-full h-full bg-gradient-to-br from-gray-800 via-gray-850 to-gray-900 flex flex-col items-center justify-center">
						...
						{:else if formState.artworkPreview}
							<img src={formState.artworkPreview} alt="Album artwork preview" class="w-full h-full object-cover" />
							<div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center">
								<div class="w-20 h-20 rounded-2xl bg-white/10 backdrop-blur-sm flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
						...
						{:else}
							<div class="w-full h-full bg-gradient-to-br from-gray-800 via-gray-850 to-gray-900 border-3 border-dashed {isDraggingArtwork ? 'border-orange-500 bg-orange-500/5' : 'border-gray-600 group-hover:border-orange-500/50'} rounded-3xl flex flex-col items-center justify-center transition-all duration-300">
								<div class="w-24 h-24 rounded-3xl bg-gradient-to-br from-orange-500/20 to-orange-600/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 border border-orange-500/30">
				...
				<input type="file" id="artwork" accept="image/jpeg,image/png" onchange={handleArtworkUpload} class="hidden" />

				<div class="flex flex-col items-center gap-4">
					<Button size="lg" onclick={() => document.getElementById('artwork')?.click()} disabled={uploadingArtwork}>
						<UploadOutline class="w-5 h-5" /> {uploadingArtwork ? 'Uploading...' : 'Browse Files'}
					</Button>
			...
			<div class="space-y-6">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-gray-400">Add your songs ({formState.songs.length} track{formState.songs.length !== 1 ? 's' : ''})</p>
					</div>
				...
				<div class="space-y-3">
					{#each formState.songs as song, index}
						<div class="group flex items-center gap-4 p-5 bg-gradient-to-br from-gray-800/80 to-gray-900/80 rounded-2xl border border-gray-700/50 hover:border-orange-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-orange-500/10">
							<div class="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-500/30 to-orange-600/30 flex items-center justify-center text-orange-400 font-bold text-lg shadow-lg border border-orange-500/20 flex-shrink-0">
								{index + 1}
							</div>
							<MusicSolid class="w-5 h-5 text-gray-500 flex-shrink-0" />
							<div class="flex-1 min-w-0">
								<p class="text-white font-semibold truncate text-lg">{song.title || 'Untitled Track'}</p>
								<p class="text-sm text-gray-400 truncate">
									{song.artists.filter((a) => a.type === 'PRIMARY').map((a) => a.name || a._id).join(', ') || 'No artists assigned'}
								</p>
							</div>
					...
					{#if formState.songs.length === 0}
						<div class="text-center py-20 border-2 border-dashed border-gray-700 rounded-3xl bg-gradient-to-br from-gray-800/30 to-gray-900/30">
							<div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-gray-700 flex items-center justify-center mx-auto mb-5 shadow-lg">
								<MusicSolid class="w-10 h-10 text-gray-500" />
							</div>
							<p class="text-white font-bold text-lg mb-2">No tracks yet</p>
							<p class="text-gray-400 text-sm mb-6 max-w-sm mx-auto">Add your first song to start building your drop</p>
		...
		{:else if formState.currentStep === 4}
			...
			<div class="space-y-8">
				<div class="text-center">
					<div class="w-16 h-16 rounded-full bg-gradient-to-r from-orange-500 to-orange-400 flex items-center justify-center mx-auto mb-4">
					...
					<div class="space-y-6">
						{#if formState.artworkPreview}
							<div class="flex justify-center lg:justify-start">
								<img src={formState.artworkPreview} alt="Album artwork" class="w-48 h-48 object-cover rounded-xl shadow-xl" />
							</div>
						{/if}
		...
		<div class="flex justify-between mt-8 pt-6 border-t border-gray-700/50">
			...
			{:else}
				<Button size="lg" onclick={submitDrop} disabled={formState.isLoading} loading={formState.isLoading}>
...
<!-- Song Modal -->
<Modal bind:open={showSongModal} title={editingSongIndex !== null ? 'Edit Song' : 'Add New Song'} size="xl">
	<div class="space-y-8">
		<!-- Step 1: Audio File Upload - Make this prominent and first -->
		<div class="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border-2 border-orange-500/20 rounded-2xl p-6">
			<div class="flex items-start gap-3 mb-4">
				...
				<div class="flex-1">
					...
					<p class="text-sm text-gray-400">Upload your track in WAV or FLAC format (16-bit/44.1kHz minimum)</p>
				</div>
			</div>
			...
			<div
				...
				onkeydown={(e) => !uploadingSong && e.key === 'Enter' && document.getElementById('song-file')?.click()}
			>
				{#if uploadingSong}
					<div class="w-16 h-16 rounded-full border-4 border-orange-500/30 border-t-orange-500 animate-spin mx-auto mb-4"></div>
				...
				{:else}
					...
					<div class="inline-flex items-center gap-2 px-4 py-2 bg-gray-800/50 rounded-lg border border-gray-700">
		...
		<div class="space-y-6">
			...
			<div class="grid gap-6">
				...
				<div>
					...
					<p class="text-xs text-gray-500 mt-2">Mark if this track contains explicit lyrics or is purely instrumental (mutually exclusive)</p>
				</div>
			</div>
		</div>
				...
				<div class="space-y-2">
					{#each tempSong.artists as artist, index}
						<div class="flex items-center gap-3 p-3 bg-gray-800/50 rounded-lg border border-gray-700/50">
							...
							<div class="flex-1 min-w-0">
								<p class="text-sm font-medium text-white truncate">{artist.name || 'Unknown Artist'}</p>
		...
		<details class="group">
			<summary class="flex items-center gap-3 cursor-pointer list-none">
				<div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
					<ChevronDownOutline class="w-4 h-4 text-gray-400 transition-transform group-open:rotate-180" />
				</div>
	...
	{#snippet footer()}
		<div class="flex items-center justify-between gap-4 w-full">
			...
			<div class="flex gap-3">
				...
				<Button onclick={saveSong} disabled={(!songFileUploaded && !editingSongIndex) || !tempSong.title || !tempSong.artists || tempSong.artists.length === 0}>
...
<Modal bind:open={showSongArtistModal} title="Add Artist to Song" size="md" class="bg-gray-800">
	<div class="space-y-4">
		...
		<ArtistSearch
			...
			label={tempSongArtist.type === 'SONGWRITER' || tempSongArtist.type === 'PRODUCER' ? 'Full Name (First Last)' : 'Artist Name'}
			placeholder={tempSongArtist.type === 'SONGWRITER' || tempSongArtist.type === 'PRODUCER' ? 'Search or enter full name...' : 'Search existing artists or create new...'}
		/>
	</div>
...
<Modal bind:open={showReleaseDateWarning} title="Short Notice Warning" size="md">
		...
		<div class="flex items-start gap-3 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
			...
			<div>
				...
				<p class="text-gray-300 text-sm">
					Drops submitted with less than 14 days notice may not be available on all platforms by your target date. Some stores require longer lead times for processing.
				</p>
			</div>
		</div>
	</div>
	...
	{#snippet footer()}
		<Button variant="secondary" onclick={() => (showReleaseDateWarning = false)}>
			Go Back
		</Button>
		<Button onclick={proceedWithShortNotice}>
			Proceed Anyway
		</Button>
	{/snippet}
</Modal>
