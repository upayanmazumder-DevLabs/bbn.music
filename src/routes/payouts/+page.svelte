...
const availableTimeFrames = $derived(
	allTimeFrames.filter(
		(tf) => payouts.length >= tf.months || tf.months === Infinity,
	),
);
...
const totalStreams = $derived(
	filteredPayouts.reduce((sum, p) => sum + p.streams, 0),
);

const avgPerStream = $derived(
	totalStreams > 0 ? totalEarnings / totalStreams : 0,
);
...
<div class="space-y-8">
			...
			<div class="flex items-center gap-1 p-1 bg-gray-800 rounded-lg">
				{#each availableTimeFrames as tf}
					<button
						...
						class="px-3 py-1.5 text-sm font-medium rounded-md transition-all {selectedMonths === tf.months
	...
	{:else if payouts.length === 0}
		<Card variant="default" padding="lg">
			<div class="text-center py-16">
				<div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-500/20 to-orange-600/10 flex items-center justify-center mx-auto mb-6">
	...
	{:else}
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
			...
			<Card variant="default" padding="md">
				<div class="flex items-center gap-4">
					<div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-blue-600/10 flex items-center justify-center">
			...
			<Card variant="default" padding="md">
				<div class="flex items-center gap-4">
					<div class="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500/20 to-purple-600/10 flex items-center justify-center">
						<ChartPieSolid class="w-6 h-6 text-purple-400" />
					</div>
					<div>
						<p class="text-sm text-gray-400">Avg. Per Stream</p>
						<p class="text-2xl font-bold text-purple-400">{formatCurrency(avgPerStream * 1000)}/1K</p>
					</div>
				</div>
			</Card>
		</div>
		...
		{#if chartData.length > 0}
			<div>
				...
				<Card variant="default" padding="lg" class="overflow-hidden">
					...
					<div class="grid items-end gap-1" style="height: 200px; grid-template-columns: repeat({chartData.length}, minmax(0, 1fr));">
						{#each chartData as data}
							...
							<div class="flex flex-col items-center justify-end h-full group min-w-0">
								<span class="text-xs font-medium text-gray-400 mb-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
								...
								></div>
								<span class="text-xs text-gray-500 font-medium mt-2 truncate w-full text-center">{formatPeriodLabel(data.label)}</span>
							</div>
						{/each}
					</div>
				</Card>
			...
			<div class="space-y-3">
				{#each payouts as payout}
					...
					<Card variant="default" padding="md" class="hover:border-gray-600 transition-colors">
						<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
							<div class="flex items-center gap-4">
								<div class="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center text-gray-400 font-mono text-sm">
