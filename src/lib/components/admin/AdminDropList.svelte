...
{:else}
	<div class="bg-gray-800/50 border border-gray-700 rounded-xl overflow-hidden">
		<table class="w-full">
			...
			<tbody class="divide-y divide-gray-700">
				{#each drops as drop}
					<tr onclick={() => openDrop(drop._id)} class="hover:bg-gray-700/50 cursor-pointer transition-colors">
						<td class="px-4 py-3">
							<p class="text-white font-medium">{drop.title || 'Untitled'}</p>
						</td>
						<td class="px-4 py-3 text-gray-400 text-sm font-mono">{drop.gtin || '-'}</td>
						<td class="px-4 py-3">
							<span class="px-2 py-1 rounded text-xs font-medium {getStatusColor(drop.type)}">{drop.type}</span>
						</td>
						<td class="px-4 py-3">
							<span class="px-2 py-1 rounded text-xs font-medium {getAccountTypeColor(drop.accountType)}">{drop.accountType}</span>
						</td>
					</tr>
				{/each}
			</tbody>
	...
	{#if hasMore}
		<div class="mt-4 text-center">
			<button onclick={onLoadMore} disabled={loadingMore} class="px-6 py-2 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:cursor-not-allowed text-white rounded-lg inline-flex items-center gap-2">
				{#if loadingMore}
