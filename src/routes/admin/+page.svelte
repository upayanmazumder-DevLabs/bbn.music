	...
	try {
		...
		if (response.data) {
			...
			if (bbnWallet?.balance) {
				bbnRevenue =
					(bbnWallet.balance.restrained || 0) +
					(bbnWallet.balance.unrestrained || 0);
			}
		}
	} catch (e) {
...
<div>
	...
	{:else}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			...
			<!-- Total Wallets Card -->
			<div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
				<h3 class="text-sm font-medium text-gray-400 uppercase tracking-wide mb-2">Total Wallets</h3>
