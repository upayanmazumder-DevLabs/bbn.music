	...
	{:else}
			...
			<table class="w-full">
				<thead class="bg-gray-900/50 border-b border-gray-700">
					<tr>
						...
						<th class="px-4 py-3 text-right text-xs font-medium text-gray-400 uppercase">Unrestrained</th>
						<th class="px-4 py-3 text-right text-xs font-medium text-gray-400 uppercase">Restrained</th>
				...
				<tbody class="divide-y divide-gray-700">
					{#each wallets as wallet}
						<tr class="hover:bg-gray-700/50">
							...
							<td class="px-4 py-3">
								<span class="px-2 py-1 rounded text-xs font-medium {getAccountTypeColor(wallet.accountType)}">{wallet.accountType}</span>
							</td>
							<td class="px-4 py-3 text-right text-green-400">{formatCurrency(wallet.balance?.unrestrained || 0)}</td>
							<td class="px-4 py-3 text-right text-yellow-400">{formatCurrency(wallet.balance?.restrained || 0)}</td>
		...
		{#if hasMore}
			<div class="mt-4 text-center">
				<button onclick={() => loadWallets(wallets.length)} disabled={loadingMore} class="px-6 py-2 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:cursor-not-allowed text-white rounded-lg inline-flex items-center gap-2">
					{#if loadingMore}
