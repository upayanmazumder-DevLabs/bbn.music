...
<label
	for={toggleId}
	class="inline-flex items-center gap-3 cursor-pointer select-none {disabled
		? 'opacity-50 cursor-not-allowed'
		: ''} {className}"
>
	<div class="relative">
		<input id={toggleId} type="checkbox" bind:checked {disabled} class="sr-only peer" />
		<div
			class="w-11 h-6 bg-gray-700 rounded-full peer {colorClasses[
				color
