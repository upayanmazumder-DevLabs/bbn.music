// This file is auto-generated by @hey-api/openapi-ts

import type { Options as ClientOptions, TDataShape, Client } from './client/fetch/index.ts';
import type { GetDropsByAdminData, GetDropsByAdminResponses, GetIdByDropsByAdminData, GetIdByDropsByAdminResponses, GetDownloadByFileByFilesByAdminData, GetDownloadByFileByFilesByAdminResponses, DeleteIdByFilesByAdminData, GetGroupsByAdminData, GetGroupsByAdminResponses, GetPayoutsByAdminData, GetPayoutsByAdminResponses, GetUploadByPayoutsByAdminData, GetQueryBySearchByAdminData, GetQueryBySearchByAdminResponses, PostSyncMappingByAdminData, GetWalletsByAdminData, GetWalletsByAdminResponses, GetIdByWalletsByAdminData, GetIdByWalletsByAdminResponses, PatchIdByWalletsByAdminData, PostEmailByAuthData, GetTokenByFromUserInteractionByAuthData, PostCodeByProviderByOauthByAuthData, GetProviderByRedirectByAuthData, PostRefreshAccessTokenByAuthData, PostRegisterByAuthData, PostResetPasswordByAuthData, PostBugTrackData, GetWebhookByStatuspageByIntegrationData, PostWebhookByStatuspageByIntegrationData, PostMusicData, GetArtworkByDropByMusicData, GetFullArtworkByDropByMusicData, PostReviewByDropByMusicData, GetServicesByDropByMusicData, PostTypeByTypeByDropByMusicData, GetArtistsByMusicData, GetArtistsByMusicResponses, PostArtistsByMusicData, PostArtistsByMusicResponses, GetDropsByMusicData, GetDropsByMusicResponses, PostDropByDropsByMusicData, PostDropByDropsByMusicResponses, GetDownloadByDropByDropsByMusicData, GetDownloadByDropByDropsByMusicResponses, GetUploadByDropByDropsByMusicData, GetIdByDropsByMusicData, GetIdByDropsByMusicResponses, PatchIdByDropsByMusicData, PostShareByDropsByMusicData, PostShareByDropsByMusicResponses, DeleteIdByShareByDropsByMusicData, GetIdByShareByDropsByMusicData, GetIdByShareByDropsByMusicResponses, GetFulldropByMusicData, GetFulldropByMusicResponses, GetIdByProviderByPublishByMusicData, GetIdByProviderByPublishByMusicResponses, GetSlugByShareByMusicData, GetArtworkBySlugByShareByMusicData, GetIdByShazamByMusicData, GetIdByShazamByMusicResponses, GetSongsByMusicData, GetSongsByMusicResponses, PostSongsByMusicData, PostSongsByMusicResponses, GetIdBySongsByMusicData, GetIdBySongsByMusicResponses, GetDownloadBySongBySongsByMusicData, GetDownloadBySongBySongsByMusicResponses, GetUploadBySongsByMusicData, GetApplicationsByOauthData, GetApplicationsByOauthResponses, PostApplicationsByOauthData, PostApplicationsByOauthResponses, GetDownloadByClientByApplicationsByOauthData, GetDownloadByClientByApplicationsByOauthResponses, DeleteIdByApplicationsByOauthData, PatchIdByApplicationsByOauthData, GetUploadByApplicationsByOauthData, PostAuthorizeByOauthData, PostTokenByOauthData, GetUserinfoByOauthData, GetUserinfoByOauthResponses, PostValidateByOauthData, GetPayoutsByPaymentData, GetPayoutsByPaymentResponses, GetIdByPayoutsByPaymentData, GetIdByPayoutsByPaymentResponses, PutPlaceholderByTasksData, PutPlaceholderByTasksResponses, GetPictureByUserByUserData, PostResendVerifyEmailByMailByUserData, PostTokenByValidateByMailByUserData, PostTokenByValidateByPhoneByUserData, GetUploadByAvatarBySetMeByUserData, PutUserByUserData, PutUserByUserResponses, PatchIdByUsersByUserData, GetWalletData, GetWalletResponses, PutWalletData, PutWalletResponses, GetChatsByWhatsappData, GetChatsByWhatsappResponses, GetIdByChatsByWhatsappData, GetIdByChatsByWhatsappResponses, GetEventByWhatsappData, PostEventByWhatsappData } from './types.gen.ts';
import { zGetDropsByAdminData, zGetDropsByAdminResponse, zGetIdByDropsByAdminData, zGetIdByDropsByAdminResponse, zGetDownloadByFileByFilesByAdminData, zGetDownloadByFileByFilesByAdminResponse, zDeleteIdByFilesByAdminData, zGetGroupsByAdminData, zGetGroupsByAdminResponse, zGetPayoutsByAdminData, zGetPayoutsByAdminResponse, zGetUploadByPayoutsByAdminData, zGetQueryBySearchByAdminData, zGetQueryBySearchByAdminResponse, zPostSyncMappingByAdminData, zGetWalletsByAdminData, zGetWalletsByAdminResponse, zGetIdByWalletsByAdminData, zGetIdByWalletsByAdminResponse, zPatchIdByWalletsByAdminData, zPostEmailByAuthData, zGetTokenByFromUserInteractionByAuthData, zPostCodeByProviderByOauthByAuthData, zGetProviderByRedirectByAuthData, zPostRefreshAccessTokenByAuthData, zPostRegisterByAuthData, zPostResetPasswordByAuthData, zPostBugTrackData, zGetWebhookByStatuspageByIntegrationData, zPostWebhookByStatuspageByIntegrationData, zPostMusicData, zGetArtworkByDropByMusicData, zGetFullArtworkByDropByMusicData, zPostReviewByDropByMusicData, zGetServicesByDropByMusicData, zPostTypeByTypeByDropByMusicData, zGetArtistsByMusicData, zGetArtistsByMusicResponse, zPostArtistsByMusicData, zPostArtistsByMusicResponse, zGetDropsByMusicData, zGetDropsByMusicResponse, zPostDropByDropsByMusicData, zPostDropByDropsByMusicResponse, zGetDownloadByDropByDropsByMusicData, zGetDownloadByDropByDropsByMusicResponse, zGetUploadByDropByDropsByMusicData, zGetIdByDropsByMusicData, zGetIdByDropsByMusicResponse, zPatchIdByDropsByMusicData, zPostShareByDropsByMusicData, zPostShareByDropsByMusicResponse, zDeleteIdByShareByDropsByMusicData, zGetIdByShareByDropsByMusicData, zGetIdByShareByDropsByMusicResponse, zGetFulldropByMusicData, zGetFulldropByMusicResponse, zGetIdByProviderByPublishByMusicData, zGetIdByProviderByPublishByMusicResponse, zGetSlugByShareByMusicData, zGetArtworkBySlugByShareByMusicData, zGetIdByShazamByMusicData, zGetIdByShazamByMusicResponse, zGetSongsByMusicData, zGetSongsByMusicResponse, zPostSongsByMusicData, zPostSongsByMusicResponse, zGetIdBySongsByMusicData, zGetIdBySongsByMusicResponse, zGetDownloadBySongBySongsByMusicData, zGetDownloadBySongBySongsByMusicResponse, zGetUploadBySongsByMusicData, zGetApplicationsByOauthData, zGetApplicationsByOauthResponse, zPostApplicationsByOauthData, zGetDownloadByClientByApplicationsByOauthData, zGetDownloadByClientByApplicationsByOauthResponse, zDeleteIdByApplicationsByOauthData, zPatchIdByApplicationsByOauthData, zGetUploadByApplicationsByOauthData, zPostAuthorizeByOauthData, zPostTokenByOauthData, zGetUserinfoByOauthData, zGetUserinfoByOauthResponse, zPostValidateByOauthData, zGetPayoutsByPaymentData, zGetPayoutsByPaymentResponse, zGetIdByPayoutsByPaymentData, zGetIdByPayoutsByPaymentResponse, zPutPlaceholderByTasksData, zPutPlaceholderByTasksResponse, zGetPictureByUserByUserData, zPostResendVerifyEmailByMailByUserData, zPostTokenByValidateByMailByUserData, zPostTokenByValidateByPhoneByUserData, zGetUploadByAvatarBySetMeByUserData, zPutUserByUserData, zPutUserByUserResponse, zPatchIdByUsersByUserData, zGetWalletData, zGetWalletResponse, zPutWalletData, zPutWalletResponse, zGetChatsByWhatsappData, zGetChatsByWhatsappResponse, zGetIdByChatsByWhatsappData, zGetIdByChatsByWhatsappResponse, zGetEventByWhatsappData, zPostEventByWhatsappData } from './zod.gen.ts';
import { client as _heyApiClient } from './client.gen.ts';

export type Options<TData extends TDataShape = TDataShape, ThrowOnError extends boolean = boolean> = ClientOptions<TData, ThrowOnError> & {
    /**
     * You can provide a client instance returned by `createClient()` instead of
     * individual options. This might be also useful if you want to implement a
     * custom client.
     */
    client?: Client;
    /**
     * You can pass arbitrary values through the `meta` object. This can be
     * used to access values that aren't defined as part of the SDK function.
     */
    meta?: Record<string, unknown>;
};

export const getDropsByAdmin = <ThrowOnError extends boolean = false>(options: Options<GetDropsByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetDropsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDropsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDropsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/drops',
        ...options
    });
};

export const getIdByDropsByAdmin = <ThrowOnError extends boolean = false>(options: Options<GetIdByDropsByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByDropsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByDropsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByDropsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/drops/{id}',
        ...options
    });
};

export const getDownloadByFileByFilesByAdmin = <ThrowOnError extends boolean = false>(options: Options<GetDownloadByFileByFilesByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetDownloadByFileByFilesByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDownloadByFileByFilesByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDownloadByFileByFilesByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/files/{fileId}/download',
        ...options
    });
};

export const deleteIdByFilesByAdmin = <ThrowOnError extends boolean = false>(options: Options<DeleteIdByFilesByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).delete<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zDeleteIdByFilesByAdminData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/files/{id}',
        ...options
    });
};

export const getGroupsByAdmin = <ThrowOnError extends boolean = false>(options?: Options<GetGroupsByAdminData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetGroupsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetGroupsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetGroupsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/groups',
        ...options
    });
};

export const getPayoutsByAdmin = <ThrowOnError extends boolean = false>(options?: Options<GetPayoutsByAdminData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetPayoutsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetPayoutsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetPayoutsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/payouts',
        ...options
    });
};

export const getUploadByPayoutsByAdmin = <ThrowOnError extends boolean = false>(options?: Options<GetUploadByPayoutsByAdminData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUploadByPayoutsByAdminData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/payouts/upload',
        ...options
    });
};

export const getQueryBySearchByAdmin = <ThrowOnError extends boolean = false>(options: Options<GetQueryBySearchByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetQueryBySearchByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetQueryBySearchByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetQueryBySearchByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/search/{query}',
        ...options
    });
};

export const postSyncMappingByAdmin = <ThrowOnError extends boolean = false>(options?: Options<PostSyncMappingByAdminData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostSyncMappingByAdminData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/sync_mapping',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getWalletsByAdmin = <ThrowOnError extends boolean = false>(options?: Options<GetWalletsByAdminData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetWalletsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetWalletsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetWalletsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/wallets',
        ...options
    });
};

export const getIdByWalletsByAdmin = <ThrowOnError extends boolean = false>(options: Options<GetIdByWalletsByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByWalletsByAdminResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByWalletsByAdminData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByWalletsByAdminResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/wallets/{id}',
        ...options
    });
};

export const patchIdByWalletsByAdmin = <ThrowOnError extends boolean = false>(options: Options<PatchIdByWalletsByAdminData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).patch<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPatchIdByWalletsByAdminData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/admin/wallets/{id}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postEmailByAuth = <ThrowOnError extends boolean = false>(options?: Options<PostEmailByAuthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostEmailByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/email',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getTokenByFromUserInteractionByAuth = <ThrowOnError extends boolean = false>(options: Options<GetTokenByFromUserInteractionByAuthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetTokenByFromUserInteractionByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/from-user-interaction/{token}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postCodeByProviderByOauthByAuth = <ThrowOnError extends boolean = false>(options: Options<PostCodeByProviderByOauthByAuthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostCodeByProviderByOauthByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/oauth/{provider}/{code}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getProviderByRedirectByAuth = <ThrowOnError extends boolean = false>(options: Options<GetProviderByRedirectByAuthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetProviderByRedirectByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/redirect/{provider}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postRefreshAccessTokenByAuth = <ThrowOnError extends boolean = false>(options?: Options<PostRefreshAccessTokenByAuthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostRefreshAccessTokenByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/refresh-access-token',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postRegisterByAuth = <ThrowOnError extends boolean = false>(options?: Options<PostRegisterByAuthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostRegisterByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/register',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postResetPasswordByAuth = <ThrowOnError extends boolean = false>(options?: Options<PostResetPasswordByAuthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostResetPasswordByAuthData.parseAsync(data);
        },
        url: '/api/@bbn/auth/reset-password',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postBugTrack = <ThrowOnError extends boolean = false>(options?: Options<PostBugTrackData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostBugTrackData.parseAsync(data);
        },
        url: '/api/@bbn/bug-track/',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getWebhookByStatuspageByIntegration = <ThrowOnError extends boolean = false>(options?: Options<GetWebhookByStatuspageByIntegrationData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetWebhookByStatuspageByIntegrationData.parseAsync(data);
        },
        url: '/api/@bbn/integration/statuspage/webhook',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postWebhookByStatuspageByIntegration = <ThrowOnError extends boolean = false>(options?: Options<PostWebhookByStatuspageByIntegrationData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostWebhookByStatuspageByIntegrationData.parseAsync(data);
        },
        url: '/api/@bbn/integration/statuspage/webhook',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postMusic = <ThrowOnError extends boolean = false>(options?: Options<PostMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getArtworkByDropByMusic = <ThrowOnError extends boolean = false>(options: Options<GetArtworkByDropByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetArtworkByDropByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/{dropId}/artwork',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getFullArtworkByDropByMusic = <ThrowOnError extends boolean = false>(options: Options<GetFullArtworkByDropByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetFullArtworkByDropByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/{dropId}/fullArtwork',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postReviewByDropByMusic = <ThrowOnError extends boolean = false>(options: Options<PostReviewByDropByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostReviewByDropByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/{dropId}/review',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getServicesByDropByMusic = <ThrowOnError extends boolean = false>(options: Options<GetServicesByDropByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetServicesByDropByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/{dropId}/services',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postTypeByTypeByDropByMusic = <ThrowOnError extends boolean = false>(options: Options<PostTypeByTypeByDropByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostTypeByTypeByDropByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/{dropId}/type/{type}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getArtistsByMusic = <ThrowOnError extends boolean = false>(options?: Options<GetArtistsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetArtistsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetArtistsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetArtistsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/artists',
        ...options
    });
};

export const postArtistsByMusic = <ThrowOnError extends boolean = false>(options?: Options<PostArtistsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostArtistsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostArtistsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPostArtistsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/artists',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getDropsByMusic = <ThrowOnError extends boolean = false>(options?: Options<GetDropsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops',
        ...options
    });
};

export const postDropByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<PostDropByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<PostDropByDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostDropByDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPostDropByDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/{dropId}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getDownloadByDropByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetDownloadByDropByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetDownloadByDropByDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDownloadByDropByDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDownloadByDropByDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/{dropId}/download',
        ...options
    });
};

export const getUploadByDropByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetUploadByDropByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUploadByDropByDropsByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/{dropId}/upload',
        ...options
    });
};

export const getIdByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetIdByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/{id}',
        ...options
    });
};

export const patchIdByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<PatchIdByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).patch<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPatchIdByDropsByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/{id}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postShareByDropsByMusic = <ThrowOnError extends boolean = false>(options?: Options<PostShareByDropsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostShareByDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostShareByDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPostShareByDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/share',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const deleteIdByShareByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<DeleteIdByShareByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).delete<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zDeleteIdByShareByDropsByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/share/{id}',
        ...options
    });
};

export const getIdByShareByDropsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetIdByShareByDropsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByShareByDropsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByShareByDropsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByShareByDropsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/drops/share/{id}',
        ...options
    });
};

export const getFulldropByMusic = <ThrowOnError extends boolean = false>(options?: Options<GetFulldropByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetFulldropByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetFulldropByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetFulldropByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/fulldrop',
        ...options
    });
};

export const getIdByProviderByPublishByMusic = <ThrowOnError extends boolean = false>(options: Options<GetIdByProviderByPublishByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByProviderByPublishByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByProviderByPublishByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByProviderByPublishByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/publish/{provider}/{id}',
        ...options
    });
};

export const getSlugByShareByMusic = <ThrowOnError extends boolean = false>(options: Options<GetSlugByShareByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetSlugByShareByMusicData.parseAsync(data);
        },
        url: '/api/@bbn/music/share/{slug}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getArtworkBySlugByShareByMusic = <ThrowOnError extends boolean = false>(options: Options<GetArtworkBySlugByShareByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetArtworkBySlugByShareByMusicData.parseAsync(data);
        },
        url: '/api/@bbn/music/share/{slug}/artwork',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getIdByShazamByMusic = <ThrowOnError extends boolean = false>(options: Options<GetIdByShazamByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByShazamByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByShazamByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByShazamByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/shazam/{id}',
        ...options
    });
};

export const getSongsByMusic = <ThrowOnError extends boolean = false>(options?: Options<GetSongsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetSongsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetSongsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetSongsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/songs',
        ...options
    });
};

export const postSongsByMusic = <ThrowOnError extends boolean = false>(options?: Options<PostSongsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostSongsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostSongsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPostSongsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/songs',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getIdBySongsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetIdBySongsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdBySongsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdBySongsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdBySongsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/songs/{id}',
        ...options
    });
};

export const getDownloadBySongBySongsByMusic = <ThrowOnError extends boolean = false>(options: Options<GetDownloadBySongBySongsByMusicData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetDownloadBySongBySongsByMusicResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDownloadBySongBySongsByMusicData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDownloadBySongBySongsByMusicResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/songs/{songId}/download',
        ...options
    });
};

export const getUploadBySongsByMusic = <ThrowOnError extends boolean = false>(options?: Options<GetUploadBySongsByMusicData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUploadBySongsByMusicData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/music/songs/upload',
        ...options
    });
};

export const getApplicationsByOauth = <ThrowOnError extends boolean = false>(options?: Options<GetApplicationsByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetApplicationsByOauthResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetApplicationsByOauthData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetApplicationsByOauthResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications',
        ...options
    });
};

export const postApplicationsByOauth = <ThrowOnError extends boolean = false>(options?: Options<PostApplicationsByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostApplicationsByOauthResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostApplicationsByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getDownloadByClientByApplicationsByOauth = <ThrowOnError extends boolean = false>(options: Options<GetDownloadByClientByApplicationsByOauthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetDownloadByClientByApplicationsByOauthResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetDownloadByClientByApplicationsByOauthData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetDownloadByClientByApplicationsByOauthResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications/{clientId}/download',
        ...options
    });
};

export const deleteIdByApplicationsByOauth = <ThrowOnError extends boolean = false>(options: Options<DeleteIdByApplicationsByOauthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).delete<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zDeleteIdByApplicationsByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications/{id}',
        ...options
    });
};

export const patchIdByApplicationsByOauth = <ThrowOnError extends boolean = false>(options: Options<PatchIdByApplicationsByOauthData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).patch<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPatchIdByApplicationsByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications/{id}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getUploadByApplicationsByOauth = <ThrowOnError extends boolean = false>(options?: Options<GetUploadByApplicationsByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUploadByApplicationsByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/applications/upload',
        ...options
    });
};

export const postAuthorizeByOauth = <ThrowOnError extends boolean = false>(options?: Options<PostAuthorizeByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostAuthorizeByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/authorize',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postTokenByOauth = <ThrowOnError extends boolean = false>(options?: Options<PostTokenByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostTokenByOauthData.parseAsync(data);
        },
        url: '/api/@bbn/oauth/token',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getUserinfoByOauth = <ThrowOnError extends boolean = false>(options?: Options<GetUserinfoByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetUserinfoByOauthResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUserinfoByOauthData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetUserinfoByOauthResponse.parseAsync(data);
        },
        url: '/api/@bbn/oauth/userinfo',
        ...options
    });
};

export const postValidateByOauth = <ThrowOnError extends boolean = false>(options?: Options<PostValidateByOauthData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostValidateByOauthData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/oauth/validate',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getPayoutsByPayment = <ThrowOnError extends boolean = false>(options?: Options<GetPayoutsByPaymentData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetPayoutsByPaymentResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetPayoutsByPaymentData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetPayoutsByPaymentResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/payment/payouts',
        ...options
    });
};

export const getIdByPayoutsByPayment = <ThrowOnError extends boolean = false>(options: Options<GetIdByPayoutsByPaymentData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByPayoutsByPaymentResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByPayoutsByPaymentData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByPayoutsByPaymentResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/payment/payouts/{id}',
        ...options
    });
};

export const putPlaceholderByTasks = <ThrowOnError extends boolean = false>(options?: Options<PutPlaceholderByTasksData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).put<PutPlaceholderByTasksResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPutPlaceholderByTasksData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPutPlaceholderByTasksResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/tasks/placeholder',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getPictureByUserByUser = <ThrowOnError extends boolean = false>(options: Options<GetPictureByUserByUserData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetPictureByUserByUserData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/user/{userId}/picture',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postResendVerifyEmailByMailByUser = <ThrowOnError extends boolean = false>(options?: Options<PostResendVerifyEmailByMailByUserData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostResendVerifyEmailByMailByUserData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/user/mail/resend-verify-email',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postTokenByValidateByMailByUser = <ThrowOnError extends boolean = false>(options: Options<PostTokenByValidateByMailByUserData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostTokenByValidateByMailByUserData.parseAsync(data);
        },
        url: '/api/@bbn/user/mail/validate/{token}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const postTokenByValidateByPhoneByUser = <ThrowOnError extends boolean = false>(options: Options<PostTokenByValidateByPhoneByUserData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostTokenByValidateByPhoneByUserData.parseAsync(data);
        },
        url: '/api/@bbn/user/phone/validate/{token}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getUploadByAvatarBySetMeByUser = <ThrowOnError extends boolean = false>(options?: Options<GetUploadByAvatarBySetMeByUserData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetUploadByAvatarBySetMeByUserData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/user/set-me/avatar/upload',
        ...options
    });
};

export const putUserByUser = <ThrowOnError extends boolean = false>(options?: Options<PutUserByUserData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).put<PutUserByUserResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPutUserByUserData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPutUserByUserResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/user/user',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const patchIdByUsersByUser = <ThrowOnError extends boolean = false>(options: Options<PatchIdByUsersByUserData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).patch<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPatchIdByUsersByUserData.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/user/users/{id}',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
    });
};

export const getWallet = <ThrowOnError extends boolean = false>(options?: Options<GetWalletData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetWalletResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetWalletData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetWalletResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/wallet/',
        ...options
    });
};

export const putWallet = <ThrowOnError extends boolean = false>(options?: Options<PutWalletData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).put<PutWalletResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPutWalletData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zPutWalletResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/wallet/',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const getChatsByWhatsapp = <ThrowOnError extends boolean = false>(options?: Options<GetChatsByWhatsappData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<GetChatsByWhatsappResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetChatsByWhatsappData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetChatsByWhatsappResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/whatsapp/chats',
        ...options
    });
};

export const getIdByChatsByWhatsapp = <ThrowOnError extends boolean = false>(options: Options<GetIdByChatsByWhatsappData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<GetIdByChatsByWhatsappResponses, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetIdByChatsByWhatsappData.parseAsync(data);
        },
        responseValidator: async (data: any) => {
            return await zGetIdByChatsByWhatsappResponse.parseAsync(data);
        },
        security: [
            {
                scheme: 'bearer',
                type: 'http'
            }
        ],
        url: '/api/@bbn/whatsapp/chats/{id}',
        ...options
    });
};

export const getEventByWhatsapp = <ThrowOnError extends boolean = false>(options?: Options<GetEventByWhatsappData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zGetEventByWhatsappData.parseAsync(data);
        },
        url: '/api/@bbn/whatsapp/event',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};

export const postEventByWhatsapp = <ThrowOnError extends boolean = false>(options?: Options<PostEventByWhatsappData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<unknown, unknown, ThrowOnError>({
        requestValidator: async (data: any) => {
            return await zPostEventByWhatsappData.parseAsync(data);
        },
        url: '/api/@bbn/whatsapp/event',
        ...options,
        headers: {
            'Content-Type': 'application/json',
            ...options?.headers
        }
    });
};