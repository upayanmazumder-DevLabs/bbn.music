...
async function saveSettings() {
	...
	try {
		...
		if (response.error) {
			throw new Error(
				(response.error as any)?.message || 'Failed to save settings',
			);
		}
...
async function changePassword(e: Event) {
	...
	try {
		...
		if (response.error) {
			throw new Error(
				(response.error as any)?.message || 'Failed to change password',
			);
		}
...
<div class="max-w-2xl mx-auto space-y-8">
	...
	{#if saved}
		<div class="flex items-center gap-3 p-4 rounded-lg bg-green-500/20 border border-green-500/30 text-green-300">
	...
	<section class="rounded-2xl p-6 bg-white/5 border border-white/10">
		...
		{#if avatarError}
			<div class="flex items-center gap-3 p-3 mb-4 rounded-lg bg-red-500/20 border border-red-500/30 text-red-300">
		...
		<div class="flex items-start gap-6 mb-6">
			<div class="relative group">
				...
				{#if !uploadingAvatar}
					<div class="absolute bottom-0 right-0 w-6 h-6 bg-gray-800 rounded-full flex items-center justify-center border-2 border-gray-900 group-hover:bg-orange-500 transition-colors">
						<svg class="w-3 h-3 text-gray-400 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		...
		<div class="space-y-4">
			<div>
				<div class="flex items-center gap-2 mb-2">
					...
					{#if emailVerified}
						<span class="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-green-500/20 text-green-400 rounded">
							<CheckCircleSolid class="w-3 h-3" />
							Verified
						</span>
					{:else}
						<span class="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-orange-500/20 text-orange-400 rounded">
				...
				{#if !emailVerified}
					{#if verificationEmailSent}
						<div class="flex items-center gap-2 mt-3 p-3 rounded-lg bg-green-500/20 border border-green-500/30 text-green-300">
			...
			<div>
				<div class="flex items-center gap-2 mb-2">
					<label for="phone-input" class="block text-sm font-medium text-gray-300">Phone Number</label>
				...
				<p class="text-xs text-gray-500 mt-1">Optional: Add your phone number for account recovery</p>
			</div>
		</div>
	...
	<section class="rounded-2xl p-6 bg-white/5 border border-white/10">
		...
		<div class="space-y-4">
			<label class="flex items-center justify-between cursor-pointer group">
				<div>
					<p class="text-white font-medium group-hover:text-orange-400 transition-colors">Drop Updates</p>
				...
				<div class="relative">
					...
					<div class="w-11 h-6 bg-white/10 rounded-full peer peer-checked:bg-orange-500 transition-colors"></div>
			...
			<label class="flex items-center justify-between cursor-pointer group">
				<div>
					<p class="text-white font-medium group-hover:text-orange-400 transition-colors">Payout Alerts</p>
			...
			<label class="flex items-center justify-between cursor-pointer group">
				<div>
					<p class="text-white font-medium group-hover:text-orange-400 transition-colors">Marketing</p>
	...
	<section class="rounded-2xl p-6 bg-white/5 border border-white/10">
		...
		<div class="space-y-4">
			<div>
				...
				{#if showPasswordChange}
					<form onsubmit={changePassword} class="mt-4 space-y-4 p-4 bg-white/5 rounded-lg border border-white/10">
