	...
	onMount(async () => {
		...
		if (type === 'reset-password' && token) {
			...
			try {
				...
				if (response.error) {
					throw new Error((response.error as any)?.message || 'Invalid or expired reset link');
				}
		...
		if (type === 'verify-email' && token) {
			...
			try {
				...
				if (response.error) {
					throw new Error(
						(response.error as any)?.message || 'Invalid or expired verification link',
					);
				}
		...
		{:else}
			<div class="glass rounded-2xl p-8 space-y-6 border border-white/10">
				...
				<div class="text-center">
					...
					<p class="mt-2 text-gray-500 dark:text-gray-400">Sign in to continue to bbn.music</p>
				</div>
				...
				<div class="relative">
					...
					<div class="relative flex justify-center text-sm">
						<span class="px-3 bg-white dark:bg-gray-900/80 text-gray-500">Or continue with</span>
					</div>
				</div>
				...
				<div class="grid grid-cols-2 gap-3" role="group" aria-label="Social sign in options">
					<button
						...
						class="flex items-center justify-center gap-2 py-3 px-4 bg-gray-100 dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-lg hover:bg-gray-200 dark:hover:bg-white/10 hover:border-gray-400 dark:hover:border-white/20 transition-all"
