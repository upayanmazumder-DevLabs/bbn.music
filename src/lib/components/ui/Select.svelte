...
const selectId =
	restProps.id ?? `select-${Math.random().toString(36).slice(2, 9)}`;
</script>

<div class="w-full">
	{#if label}
		<label for={selectId} class="block text-sm font-medium text-white mb-2">
			{label}
	...
	<div class="relative">
		<select
			...
			bind:value
			class="
				w-full px-4 py-2.5 rounded-lg text-white appearance-none cursor-pointer
