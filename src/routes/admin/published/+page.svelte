	...
	async function loadDrops(offset = 0) {
		...
		try {
			const response = await getDropsByAdmin({
				query: { type: 'PUBLISHED', _limit: LIMIT, _offset: offset },
...
<div>
	<h1 class="text-2xl font-bold text-white mb-6">Published Drops</h1>
	<p class="text-gray-400 mb-6">Drops that have been published to stores.</p>
