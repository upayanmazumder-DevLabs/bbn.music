...
const walletNavItems: NavItem[] = [
	{ href: '/wallet', label: 'Overview', exact: true },
];
...
<div class="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-black to-gray-900">
							...
							{#each navItems as item}
								...
								<a
									...
									class="px-4 py-2 rounded-lg transition-all duration-200 {active
										? currentApp === 'admin' ? 'bg-red-500/20 text-red-400' : currentApp === 'wallet' ? 'bg-green-500/20 text-green-400' : 'bg-orange-500/20 text-orange-400'
				...
				<div class="flex items-center space-x-3">
					...
					{#if $auth.isAuthenticated}
						...
						<div class="relative" data-user-menu>
							<button
								...
								class="w-8 h-8 rounded-full flex items-center justify-center hover:opacity-90 transition-opacity overflow-hidden {currentApp === 'admin' ? 'bg-gradient-to-r from-red-500 to-red-400' : currentApp === 'wallet' ? 'bg-gradient-to-r from-green-500 to-green-400' : 'bg-gradient-to-r from-orange-500 to-orange-400'}"
							...
							>
								...
								{:else}
									<span class="text-white text-sm font-semibold">
										{$auth.user?.profile.username ? $auth.user.profile.username[0].toUpperCase() : 'U'}
									</span>
								{/if}
							</button>
							...
							{#if showUserMenu}
								...
								>
									<div class="px-4 py-2 border-b border-gray-700">
										<p class="text-sm text-white font-semibold">{$auth.user?.profile.username || 'User'}</p>
										<p class="text-xs text-gray-400">{$auth.user?.profile.email || ''}</p>
									</div>
									<a href="/settings" role="menuitem" class="block px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white">
										Settings
									</a>
					...
					{:else}
						...
						<div class="hidden md:flex items-center space-x-2">
							<a href="/signin" class="text-gray-300 hover:text-white px-3 py-1.5 text-sm transition-colors">
								Sign In
							</a>
							<a href="/register" class="bg-gradient-to-r from-orange-500 to-orange-400 text-white px-4 py-1.5 rounded-full text-sm hover:opacity-90 transition-opacity">
								Sign Up
							</a>
						</div>
					{/if}
					...
					{#if !isPublicRoute && $auth.isAuthenticated}
						<button onclick={toggle} class="md:hidden p-2 text-gray-400 hover:text-white transition-colors">
			...
			{#if !hidden && !isPublicRoute && $auth.isAuthenticated}
				<div class="md:hidden pb-4">
					<div class="flex flex-col space-y-1">
						...
						<a href="/settings" class="text-gray-300 hover:text-white hover:bg-white/10 px-4 py-2 rounded-lg transition-all duration-200">
							Settings
						</a>
						<button
							onclick={handleLogout}
	...
	<!-- Main Content -->
	<main class="{isSharePage ? 'flex-1' : 'flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full'}">
		{#if isPublicRoute || authChecked}
			{@render children()}
		{:else}
			<div class="flex items-center justify-center min-h-[50vh]">
				<div class="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin {currentApp === 'admin' ? 'border-red-500' : currentApp === 'wallet' ? 'border-green-500' : 'border-orange-500'}"></div>
			</div>
		{/if}
	</main>
			...
			<div class="flex flex-col space-y-6">
				...
				<div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
					...
					<div class="flex space-x-6">
						<a href="https://facebook.com" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-white transition-colors" aria-label="Facebook">
						...
						<a href="https://instagram.com" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-white transition-colors" aria-label="Instagram">
							<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
